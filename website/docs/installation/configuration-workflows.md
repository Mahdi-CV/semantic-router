...
`POST /config/router/validate` endpoint with `compare_to_active: true`. The
endpoint must expose validation contract `v1` from #3477. Use a base
configuration that represents the Router's active snapshot so the structured
diff compares the intended documents.
...
Proposal intents contain a goal and maintained fragment IDs. They cannot carry
arbitrary configuration fields, deployment actions, or credentials. Conflicting
fragments, unsupported IDs, invalid candidates, and unsupported validation
contracts fail explicitly. Generation never changes the base file or calls a
Router update, apply, deployment, or activation endpoint.
