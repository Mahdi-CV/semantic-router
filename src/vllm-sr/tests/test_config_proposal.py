...
def canonical_validation(monkeypatch: pytest.MonkeyPatch):
    def validate(
        ...
        assert endpoint is None or endpoint
        assert timeout > 0
        assert token_env
        return {
            "valid": True,
...
def test_provenance_uses_digests_and_stable_ids_not_local_paths(tmp_path: Path):
    ...
    assert provenance["generator"]["name"] == "vllm-sr"
    assert provenance["generator"]["version"]
    assert "schema_version" not in provenance["intent"]
...
def test_intent_rejects_unknown_fields(tmp_path: Path):
    base, _, fragment_root = _fixture(tmp_path)
    intent = tmp_path / "unsupported-field.yaml"
    intent.write_text(
        yaml.safe_dump(
            {
                "schema_version": "vllm-sr/config-proposal-intent/v1",
                "goal": "Add bounded routing behavior.",
                "fragments": ["signal/complexity/escalation"],
            },
            sort_keys=False,
        ),
        encoding="utf-8",
    ...
    with pytest.raises(
        ConfigProposalError, match=r"unsupported fields: schema_version"
    ):
        propose_config(
            base_config=base,
            intent_path=intent,
...
def test_validation_request_uses_only_side_effect_free_endpoint(
        ...
        def validate_config(
            ...
            *,
            ...
            calls[-1].update(
                {
                    "yaml": yaml_text,
                    "compare_to_active": compare_to_active,
                }
            )
    ...
    monkeypatch.setattr(config_proposal, "RouterManagementClient", Client)
    base, intent, fragment_root = _fixture(tmp_path)
    ...
    propose_config(
        ...
        output_dir=tmp_path / "proposal",
        validation_endpoint="http://router:8080/",
        validation_token_env="ROUTER_TOKEN",
        validation_request=_REAL_VALIDATION_REQUEST,
    )

    assert len(calls) == 1
    assert calls[0]["base_url"] == "http://router:8080/"
    assert calls[0]["timeout"] == 10.0
    assert calls[0]["token_env"] == "ROUTER_TOKEN"
    assert calls[0]["compare_to_active"] is True
    assert calls[0]["yaml"]
...
def test_cli_defaults_to_maintained_fragment_root(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    ...
    result = CliRunner().invoke(
        ...
        [
            ...
            "propose",
            "--config",
            str(repository_root / "config/recipes/knowledge/config.yaml"),
            "--intent",
            str(repository_root / "config/proposals/keyword-signals.yaml"),
            "--output",
            str(output),
        ],
    )
...
def test_cli_requires_explicit_output(tmp_path: Path):
    base, intent, fragment_root = _fixture(tmp_path)
    ...
    assert not (tmp_path / "config-proposal").exists()
