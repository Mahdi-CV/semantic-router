...
ValidationRequest = Callable[[str, str | None, float, str], dict[str, Any]]
...
def propose_config(
    *,
    ...
    validation_token_env: str = "VSR_MGMT_TOKEN",
...
def _request_router_validation(
    ...
    try:
        response = RouterManagementClient(
            ...
            timeout=timeout,
            token_env=token_env,
        ).validate_config(proposed_yaml, compare_to_active=True)
    except ValueError as error:
        raise ConfigProposalError(str(error)) from error
